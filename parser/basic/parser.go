...
// ParseQuery :
func (p Parser) ParseQuery(query map[string]interface{}, assign interface{}) error {
	t := reflect.TypeOf(assign)
	if t.Kind() != reflect.Ptr && t.Elem().Kind() != reflect.Struct {
		return errors.New("Query interface must be struct")
	}
...
func parseQuery(query interface{}, field reflect.Value) error {
